...
interface BarangKeluarTableProps {
  data: BarangKeluarWithRelasi[];
  onAdd: () => void;
}

export function BarangKeluarTable({ data, onAdd }: BarangKeluarTableProps) {
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[12px] font-bold uppercase tracking-wider text-slate-500">Barang</th>
                <th className="px-6 py-4 text-[12px] font-bold uppercase tracking-wider text-slate-500">Jumlah Keluar</th>
                <th className="px-6 py-4 text-[12px] font-bold uppercase tracking-wider text-slate-500">Total HPP</th>
                <th className="px-6 py-4 text-[12px] font-bold uppercase tracking-wider text-slate-500 text-right">User / Penanggung Jawab</th>
              </tr>
            </thead>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-1.5 text-[12px] text-slate-500 mt-1">
                        <Calendar size={12} />
                        <span>{formatDate(item.tanggal_keluar)}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[12px] text-slate-500">{item.barang?.barang_code}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-[13px] font-bold bg-red-50 text-red-600 border border-red-100">
                      - {item.jumlah_keluar}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-[14px] font-bold text-slate-800">{formatRupiah(item.total_hpp)}</span>
                      <span className="text-[11px] text-slate-400">@ {formatRupiah(item.hpp)}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-[13px] font-medium text-slate-600">{item.users?.name}</span>
                      {item.keterangan && <span className="text-[11px] text-slate-400 mt-1 line-clamp-1 max-w-[200px]">{item.keterangan}</span>}
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div key={item.id} className="bg-white/80 backdrop-blur-xl border border-white/80 rounded-2xl p-5 shadow-[0_4px_16px_-8px_rgba(0,0,0,0.05)] flex flex-col gap-3">
            <div className="flex justify-between items-start">
              <div className="flex flex-col">
                ...
                <span className="text-[11px] text-slate-500 mt-0.5">{item.keterangan || 'Tidak ada keterangan'}</span>
              </div>
              <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-[13px] font-bold bg-red-50 text-red-600">
                - {item.jumlah_keluar}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex flex-col items-end">
                <span className="text-[10px] text-slate-400">Total HPP</span>
                <span className="text-[14px] font-bold text-slate-800">{formatRupiah(item.total_hpp)}</span>
              </div>
            </div>
          </div>
